...
`_greedy_hard_split()` 使用 sqrt 加权抽样（样本量越大的表位越容易被选入测试集），而非简单的等概率随机。测试对比了两种方法在 seeds 42-55 下的表现：

| 方法 | 测试表位数（均值） | AUC 范围 | 稳定种子占比 |
|:----|:----------------:|:--------:|:----------:|
| sqrt 加权 greedy | 43-90 | 0.578-0.921 | ~11/14 |
| 等概率随机 | 176 | 0.496-0.761 | ~2/14 |

等概率随机每次选出 ~176 个测试表位（20% of 881），但 VDJdb 中各表位样本量差异巨大（少数表位有上万条记录且几乎全为正样本），导致测试集正负比极端波动，AUC 接近随机。greedy 方法通过偏向大表位来稳定样本量分布，是复现中选择该方案的原因。

对比脚本：`scripts/test_equal_prob_split.py`
